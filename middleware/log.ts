...
export default function logMiddleware(middleware:Middleware) {
    ...
    return async (request:any) => {
        ...
        // 打印请求信息和请求耗时
        console.log(`[${new Date().toLocaleString()}] ${request.nextUrl.pathname} ${request.nextUrl.search} ${request.method} ${request.headers.get("user-agent")} Duration(Not working): ${durationInMs.toFixed(2)}ms (${durationInUs.toFixed(2)}μs)`);
        return result;
    };
}